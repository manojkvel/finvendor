...
g_l = [];
g_l["MONTHS"] = [ "Janaury", "February", "March", "April", "May", "June",
...
/*
 * The image path can be changed easily , however a few important safety steps
 * must take place!
 * 
 * CSS as a rule-of-thumb is always looking for relative image paths to where
 * the CSS file resides. Meaning , if we place the css document of JsDatePick
 * somewhere else Since some of the elements inside the CSS have
 * background:url(img/someimage.png);
 * 
 * The system will try to look for a file under the same folder where the CSS
 * file is. So pay careful attention when moving the CSS file somewhere else as
 * the images folder must be relative to it. If you want to put the CSS document
 * somewhere else and the images somewhere else - you HAVE to look and replace
 * each background:url(img/someimage.png); to the new path you desire.
 * 
 * That way you ensure risk free operation of images. For any further questions
 * or support about this issue - please consider the feedback form at
 * javascriptcalendar.org Thank you!
 */
...
JsDatePick.getCalInstanceById = function(id) {
	return g_arrayOfUsedJsDatePickCals[parseInt(id, 10)];
};
...
JsDatePick.prototype.setConfiguration = function(aConf) {
	this.oConfiguration.isStripped = (aConf["isStripped"] != null) ? aConf["isStripped"]
			: false;
	this.oConfiguration.useMode = (aConf["useMode"] != null) ? aConf["useMode"]
			: 1;
	this.oConfiguration.selectedDate = (aConf["selectedDate"] != null) ? aConf["selectedDate"]
			: null;
	this.oConfiguration.target = (aConf["target"] != null) ? aConf["target"]
			: null;
	this.oConfiguration.yearsRange = (aConf["yearsRange"] != null) ? aConf["yearsRange"]
			: [ 1971, 2100 ];
	this.oConfiguration.limitToToday = (aConf["limitToToday"] != null) ? aConf["limitToToday"]
			: false;
	this.oConfiguration.field = (aConf["field"] != null) ? aConf["field"]
			: false;
	this.oConfiguration.cellColorScheme = (aConf["cellColorScheme"] != null) ? aConf["cellColorScheme"]
			: "ocean_blue";
	this.oConfiguration.dateFormat = (aConf["dateFormat"] != null) ? aConf["dateFormat"]
			: "%m-%d-%Y";
	this.oConfiguration.imgPath = (g_jsDatePickImagePath.length != null) ? g_jsDatePickImagePath
			: "img/";
	this.oConfiguration.weekStartDay = (aConf["weekStartDay"] != null) ? aConf["weekStartDay"]
			: 1;
...
JsDatePick.prototype.makeCalendar = function() {
	...
	closeButton.onmouseover = function() {
		var gRef = JsDatePick.getCalInstanceById(this
				.getAttribute("globalNumber"));
	...
	if (this.oConfiguration.useMode == 2) {
		if (this.oConfiguration.target != false) {
			if (typeof (JsgetElem(this.oConfiguration.target)) != null) {
				...
				aFunc = new Function("g_arrayOfUsedJsDatePickCals["
						+ this.globalNumber
	...
	if (this.lastPickedDateObject) {
		...
		if (aField.value.trim().length == 0) {
			...
			if (aField.value.trim().length > 5) {
				...
				if (tflag) {
					for (i = 0; i < 12; i++) {
						if (g_l["MONTHS"][i].substr(0, 3).toUpperCase() == array[mI]
								.toUpperCase()) {
							mI = i + 1;
							break;
						}
					...
					for (i = 0; i < 12; i++) {
						if (g_l["MONTHS"][i].toLowerCase() == array[mI]
								.toLowerCase()) {
							mI = i + 1;
							break;
						}
...
JsDatePick.prototype.senseDivider = function(aStr) {
	return aStr.replace("%d", "").replace("%j", "").replace("%m", "").replace(
...
JsDatePick.prototype.isAvailable = function(y, m, d) {
	...
	if (d > this.oCurrentDay.day && m == this.oCurrentDay.month
			&& y == this.oCurrentDay.year) {
		return false;
	}
...
JsDatePick.prototype.populateMainBox = function(aMainBox) {
	...
	this.flag_DayMarkedBeforeRepopulation = false;
	this.setControlBarText(this.monthsTextualRepresentation[cmpMonth] + ", "
			+ this.currentYear);
	...
	while (oDay.getMonth() == cmpMonth) {
		...
		aDayDiv.onmouseover = function() {
			var gRef = JsDatePick.getCalInstanceById(this
					.getAttribute("globalNumber")), currentColorScheme;
			...
			if (parseInt(this.getAttribute("isToday")) == 1) {
				...
				this.style.background = "url(" + gRef.oConfiguration.imgPath
						+ currentColorScheme
		...
		if (this.isSelectedDay(oDay.getDate())) {
			...
			if (parseInt(aDayDiv.getAttribute("isToday")) == 1) {
				...
				aDayDiv.style.background = "url(" + this.oConfiguration.imgPath
			...
			if (parseInt(aDayDiv.getAttribute("isToday")) == 1) {
				if (disabledDayFlag) {
					...
					aDayDiv.style.background = "url("
							+ this.oConfiguration.imgPath
							+ this.oConfiguration.cellColorScheme
							+ "_dayNormal.gif) left top no-repeat";
				} else {
...
JsDatePick.prototype.getSelectedDayFormatted = function() {
	if (this.flag_aDayWasSelected) {
		...
		dateStr = dateStr.replace("%d", this.selectedDayObject.day.toString()
				.strpad());
		dateStr = dateStr.replace("%j", this.selectedDayObject.day);

		dateStr = dateStr.replace("%m", this.selectedDayObject.month.toString()
				.strpad());
		dateStr = dateStr.replace("%M",
				g_l["MONTHS"][this.selectedDayObject.month - 1].substr(0, 3)
						.toUpperCase());
		dateStr = dateStr.replace("%n", this.selectedDayObject.month);
		dateStr = dateStr.replace("%F",
				g_l["MONTHS"][this.selectedDayObject.month - 1]);

		dateStr = dateStr.replace("%Y", this.selectedDayObject.year);
		dateStr = dateStr.replace("%y", this.selectedDayObject.year.toString()
				.substr(2, 2));
...
JsDatePick.prototype.setDaySelection = function(anElement) {
	...
	if (this.flag_DayMarkedBeforeRepopulation) {
		...
		if (parseInt(this.lastMarkedDayObject.getAttribute("isToday")) == 1) {
			...
			this.lastMarkedDayObject.style.background = "url("
					+ this.oConfiguration.imgPath + currentColorScheme
					+ "_dayNormal.gif) left top no-repeat";
		} else {
	...
	if (parseInt(anElement.getAttribute("isToday")) == 1) {
		...
		anElement.style.background = "url(" + this.oConfiguration.imgPath
				+ currentColorScheme + "_dayDown.gif) left top no-repeat";
	} else {
