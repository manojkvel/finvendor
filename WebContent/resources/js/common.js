...
function getResearchSubAreaCompanyDetailsList() {
	$.ajax({
		type: 'GET',
		url:  "getJsonReferenceData?referenceDataType=ResearchAreaCompanyDetails",
		cache:false,
		success : function(response) {
			window.localStorage.setItem('ResearchAreaCompanyDetails', JSON.stringify(response));
		},
		error : function(data, textStatus, jqXHR) {
				//alert('Error: '+data+':'+textStatus);
			}
...
function getResearchSubAreaCompanyDetailsListMapping(rcResearchSubAreaSelector, rcResearchReportForSelector) {
	var researchSubAreaCompanyDetailsList = JSON.parse(window.localStorage.getItem('ResearchAreaCompanyDetails'));
	var rcResearchSubAreaId = 0;
	if($("select[name=" + rcResearchSubAreaSelector + "]").length == 0) {
		rcResearchSubAreaSelector = $("#" + rcResearchSubAreaSelector).attr("data-id");
	} else {
	}

	var $option='';
    for (var val in researchSubAreaCompanyDetailsList) {
    	for(var id in rcResearchSubAreaId) {
	        if (researchSubAreaCompanyDetailsList[val].parentId == rcResearchSubAreaId[id]) {
	        	$option += "<option value='" + researchSubAreaCompanyDetailsList[val].id + "'>" + researchSubAreaCompanyDetailsList[val].name + "</option>";
	        }    		
    	}
    }
    $("select#" + rcResearchReportForSelector +"").empty();
    $("select[name=" + rcResearchReportForSelector + "]").append($option);	
    $("select[name=" + rcResearchReportForSelector + "]").selectpicker('refresh');

    /*if(getResearchAreaMultipleListById(rcResearchSubAreaId)[0].toLowerCase() == ("debt market research") || getResearchAreaMultipleListById(rcResearchAreaId)[0].toLowerCase() == ("index/fund/etf research")) {
    	$("#stocksFundsIssuesCovered").val("");
    	$("#stocksFundsIssuesCovered_parent").show();
    } else {
    	$("#stocksFundsIssuesCovered_parent").hide();
  	...
  	if($("#vendor_my_offerings").is(':visible')){ 
	    ...
	    if(getResearchAreaMultipleListById(rcResearchSubAreaId)[0].toLowerCase() == ("equity research")) {
	    	$("#vo_target_price").parent().show();
	    } else {
	    	$("#vo_target_price").parent().hide();
	    }
