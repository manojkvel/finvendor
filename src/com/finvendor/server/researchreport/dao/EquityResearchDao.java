...
public class EquityResearchDao extends AbsResearchReportDao {
	...
	public List<EquityResearchResult> findResearchReportTableData(ResearchReportFilter filter) throws RuntimeException {
		...
		try {
			String baseSql = SqlEnum.EQUITY_RESEARCH.valueOf();
			String newSql = preapreSql(baseSql,equityFilter);
			SQLQuery query = this.sessionFactory.getCurrentSession().createSQLQuery(newSql);
			...
			for (Object[] row : rows) {
				...
				equityRR.setBroker(row[9] != null ? row[9].toString() : "");
				...
				equityRR.setReport(row[18] != null ? row[18].toString() : "");
