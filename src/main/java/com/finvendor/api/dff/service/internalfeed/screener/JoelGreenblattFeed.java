...
import com.finvendor.api.dff.service.internalfeed.screener.dto.JoelEbitAndEnterpriseDto;
import com.finvendor.api.dff.service.internalfeed.screener.dto.JoelRotcDto;
...
public class JoelGreenblattFeed extends AbstractScreenerFeed {
    private static final String INSERT_QUERY = "insert into strategy_joel_greenblatt values(?,?,?,?,?,?,?,?,?)";
    private static final String DELETE_QUERY = "delete from  strategy_joel_greenblatt";

    @Override
    public boolean processAndFeed() throws Exception {
        deleteAllRecordsFromStrategyTable("JOEL GREENBLATT STRATEGY", DELETE_QUERY);
    ...
    private void insertFeed(JoelRotcDto joelRotcDto, JoelEbitAndEnterpriseDto joelEbitAndEnterpriseDto) {
        ...
        String revenue = joelEbitAndEnterpriseDto.getRevenue();
        String operatingProfitMargin = joelEbitAndEnterpriseDto.getOperatingProfitMargin();
        ...
        sqlQuery.setParameter(4, StringUtils.isEmpty(revenue) ? "-" : revenue);
        sqlQuery.setParameter(5, StringUtils.isEmpty(operatingProfitMargin) ? "-" : operatingProfitMargin);

        sqlQuery.setParameter(6, StringUtils.isEmpty(mcap) ? "-" : mcap);
        sqlQuery.setParameter(7, StringUtils.isEmpty(totalDebt) ? "-" : totalDebt);
        sqlQuery.setParameter(8, StringUtils.isEmpty(cashAndCashEquiv) ? "-" : cashAndCashEquiv);
