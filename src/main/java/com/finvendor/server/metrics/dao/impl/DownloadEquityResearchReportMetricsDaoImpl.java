...
@Repository
public class DownloadEquityResearchReportMetricsDaoImpl extends GenericDao<DownloadEqtyResearchReportsMetrics>
		implements IMetricsDao {

	private static final String LOCAL_DATE_FORMAT_YYY_MM_DD = "yyy/MM/dd";
	private static final String downloadEqtyRRMetricsSql = "select user_name username, web_request WebRequest,sum(count) TotalHitCount, monthname(local_date) Month,  year(local_date) Year from download_eqty_research_report_metrics group by user_name, monthname(local_date), year(local_date) order by user_name";
	@Autowired
	...
	public void increaseCount(String userName, String request) throws RuntimeException {
		try {
			...
			org.hibernate.Query query = commonDao
					.getNamedQuery(DownloadEqtyResearchReportsMetrics.METIRCS_BY_REQUEST_NAME_NAMED_QUERY, paramMap);
	...
	public List<MetricsDto> getRequestMetrics() throws RuntimeException {
		...
		try {
			SQLQuery sqlQuery = commonDao.getNativeQuery(downloadEqtyRRMetricsSql, null);
